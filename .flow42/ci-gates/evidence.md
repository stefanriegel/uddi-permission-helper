...
- The legacy stage spelling `verification` was normalized append-only to
  canonical `verifying`, then advanced through `pr-ready` to `ci-running`.
  `ci_state` is `green`, and the next action is `awaiting-current-review`. No
  product or workflow edit, commit, push, PR/Forge write, merge, deployment, or
